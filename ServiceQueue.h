...
class ServiceQueue
{
 ...
 public:
  ...
  void update(int clock) 
  {
	...
	plane->enter_departure_time = max_service_time;
	...
	int num_served = landing_queue->get_num_served() + 1;
	
	landing_queue->num_served = num_served;
	landing_queue->total_wait = wait_time;

	// FIXME: update the start_service_time attribute for the plane
	
	plane->start_service_time = min_service_time;
	...
	*/
	...
	my_random.next_int(min_service_time);
